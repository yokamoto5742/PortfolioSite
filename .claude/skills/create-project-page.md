...
```
スキルのcreate-project-pageを使用してください。https://github.com/{owner}/{repo}
```
...
### 6. docs/index.md を更新する

`docs/index.md` の `<div class="grid cards" markdown>` ブロック内に以下のカードエントリを追加する:
...
-   **{リポジトリ名}**
    ...
    {一行の説明文（キャッチコピーをベースに動詞を加えた文）}

    [:octicons-arrow-right-24: 詳細](projects/{repo}/index.md)
```

- 既に同じパスのエントリがある場合は追加しない
- プレースホルダー（アプリ名 / appname など）のエントリがある場合はそれを置き換える
...
docs/index.md のプロジェクト一覧に "{リポジトリ名}" を追加しました。
```
